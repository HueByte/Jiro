...
$config = @{
    ...
    JiroCloudApiKey = ""
    JiroCloudGrpcServerUrl = "https://localhost:5001"
    JiroCloudGrpcMaxRetries = 3
    JiroCloudGrpcTimeoutMs = 30000
    JiroCloudWebSocketHubUrl = "https://localhost:5001/instanceHub"
...
if ($Default) {
    ...
    # Main API Key
    $apiKey = Read-Host "Jiro Instance API Key [auto-generated]"
    if ($apiKey) { 
    ...
    $jiroCloudApiKey = Read-Host "JiroCloud API Key [auto-generated]"
    if ($jiroCloudApiKey) { 
        $config.JiroCloudApiKey = $jiroCloudApiKey 
    } else {
        $config.JiroCloudApiKey = New-SecureString -Length 32
        Write-Host "Generated: $($config.JiroCloudApiKey)" -ForegroundColor DarkGray
    }
...
elseif ($NonInteractive) {
    ...
    Write-Host "  - Jiro Instance API Key"
    Write-Host "  - JiroCloud API Key"
    Write-Host ""
...
# Chat Configuration
$(if ($config.ChatAuthToken) { "JIRO_Chat__AuthToken=$($config.ChatAuthToken)" } else { "# JIRO_Chat__AuthToken=your-openai-api-key-here" })
...
if (-not $config.ChatAuthToken) {
    ...
    Write-Host "    - Update 'JIRO_Chat__AuthToken' in .env file"
    Write-Host "    - Update 'Chat.AuthToken' in appsettings.json"
    Write-Host ""
    Write-Host "  Generated secure values:"
    Write-Host "  • Jiro Instance API Key: $($config.ApiKey)"
    Write-Host "  • JiroCloud API Key: $($config.JiroCloudApiKey)"
    Write-Host ""
